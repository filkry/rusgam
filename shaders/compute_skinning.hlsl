...
// -- input
StructuredBuffer<matrix> joint_model_to_world_transforms : register(t0);
...
void main(uint3 global_thread_id : SV_DispatchThreadID)
{
    ...
    matrix vertmat = mul(joint_weights[0], joint_model_to_world_transforms[joints[0]]) +
                     mul(joint_weights[1], joint_model_to_world_transforms[joints[1]]) +
                     mul(joint_weights[2], joint_model_to_world_transforms[joints[2]]) +
                     mul(joint_weights[3], joint_model_to_world_transforms[joints[3]]);
