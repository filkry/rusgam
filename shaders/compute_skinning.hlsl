...
// -- input
StructuredBuffer<matrix> joint_bind_to_cur : register(t0);
...
void main(uint3 global_thread_id : SV_DispatchThreadID)
{
    ...
    matrix vertmat = mul(joint_weights[0], joint_bind_to_cur[joints[0]]) +
                     mul(joint_weights[1], joint_bind_to_cur[joints[1]]) +
